...
> 不支持降权）→ **correct 支主案 ω=1（等权）+ ω 消融轴**；**(c) 在跑**。
> C 存活（R-a K=50=−0.214，正交/不相关，稳健确认）= **wrong 支 C′ 部件**。**训练全部冻结。**
...
- **truncated 支｜V(t) 分派**。verifier 失明；用 V(t) 截断点读数把 truncated rollout 分派到上两支。
  默认由 triage (c) 裁：**(c) 不成立 → 保守按 wrong 处理**。（(c) 与 correct-支之争独立。）
...
- **解冻条件（维持，缺一不可）**：**(c) 判定落地**（AUC≥0.65 用 V(t) 分派 / 否则保守按 wrong）
  + **用户明示发令**。（裁决二已收案：correct 支 ω=1 主案 + ω 消融轴，不再是解冻门。）
- **最小实验矩阵**（1.7B 单 seed）：

  | # | run | 训练 | 说明 |
  |--:|---|:--:|---|
  | 1 | 主案（B + C′ + ω=1） | ✓ | 全局漂移扣除 + 错支 C′ + correct 等权 |
  | 2 | 去 B（C′ + ω=1） | ✓ | 漂移扣除消融 |
  | 3 | 去 C′（B + ω=1） | ✓ | 错支手术消融 |
  | 4 | ω=0.5（B + C′） | ✓ | correct 降权档消融 |
  | 5 | ω=0（B + C′）〔算力允许〕 | ✓ | 剂量-响应端点，预期掉分 |
  | B1 | base（未训练 Qwen3-1.7B） | ✗ | 仅 eval 基线 |
  | B2 | 官方 OPSD（复用 `qwen31b_repro_3xh200_gb30`） | ✗ | 仅 eval 基线，不重训 |

- **预计总卡时**（供过目，2 GPU/训练 run，gb30 保持 2×5×3）：
  - 训练：run 1–4 各 ~1.5h × 2 GPU = **~12 GPU-h**；加 ω=0（run 5）= **~15 GPU-h**。
    （每 run 含每步 +1 腐蚀 teacher forward，96.6ms 基准 @1024，~75–95min/run。）
  - eval：5 训练 run + B1 + B2 = 7 条 × 2 ckpt（100/150；base 单点）× (AIME24/25 avg@12 +
    MATH500 avg@4)，1 GPU/eval job，估 ~1.5h/条 ≈ **~10–12 GPU-h**。
  - **合计 ≈ 22–27 GPU-h**（8 卡共享池上 **≤ 一夜**）。4B 迁移待 1.7B 侦察后另议。
