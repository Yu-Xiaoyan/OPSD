...
> **状态：C 存活（初判）/ 稳健性确认 pending。** Q-d 字面命中预注册 ≤0.4（全词表 Spearman
> 0.115/0.140、top-mass overlap 0.04），战略层判 C 存活；**稳健性待 R-a top-K 并集域 Spearman
> 确认**（`direction_axis_recon.md`）。**本线一切训练实验冻结，待 top-K 确认后放行。**
...
- **放行条件**：R-a 主判 K=50 的 top-K 并集域 Spearman **≤0.4**（确认正交）→ 解冻本矩阵；
  若 ≥0.7 或中间带 → 按预注册回人判，矩阵维持冻结。
