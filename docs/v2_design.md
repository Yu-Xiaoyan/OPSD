...
### (A) 三路分诊（资源分配）
- **correct 支｜大幅降权 / 跳过**。原 rationale（学生已会、信号以漂移为主）**未获支撑**——
  triage (a) 证伪（漂移份额桶间不变）。**降权失据 → 裁决二另立探针重审**（按桶测**教学含量**：
  teacher-student 分歧绝对量 / δ 正向质量，非漂移份额）。**重审判定落地前，correct 支降权悬置。**
...
- **放行条件（三者缺一不可，2026-07-15 更新）**：
  1. **裁决二判定落地**——correct-支 rationale 重审（教学含量探针 (i)）出结果并据判据定架构
     （≥1.5 降权获据 / <1.2 取消降权改等权 / 中间人判）。
  2. **(c) 判定落地**——truncated 支分派方案确定（AUC≥0.65 用 V(t) 分派 / 否则保守按 wrong）。
  3. **用户明示发令**。
  - 注：原"(a)(b) 通过"已被 (a) FAIL + 裁决二重审取代；(b) 边际通过（1.55×）保留但须连带 (ii) 复测。
