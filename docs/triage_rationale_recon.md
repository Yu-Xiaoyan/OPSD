...
- **判决：wrong/correct = 1.27× ∈ (1.2, 1.5) → 轻降权，人判**。
  correct 桶教学含量确低于 wrong，但**仅低 ~21%**（1/1.27），**远非原"大幅降权/跳过"所设的悬殊**。
  → **correct 支应"轻降权"而非"大幅降权/跳过"或"完全等权"；确切权重待人判。**
- **架构落地建议**：correct 支由"大幅降权/跳过"改为**轻降权**（如 loss 权重 ~0.5–0.8，具体待定）；
  与"全局漂移扣除"（B 部件）叠加时，漂移去毒不再由 correct-支降权替代承担。
