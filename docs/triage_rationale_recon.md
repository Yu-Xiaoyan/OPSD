...
- **判决（按预注册，收案）**：AUC 0.648 判 **FAIL，不挪线** → **truncated 支保守默认（按 wrong 处理）**。
  V(t) 截断点读数弱预测力（0.648>0.5）未达分派门槛。
- **副产品入档（max_len 消融动机）**：续写后 **49% truncated 达对**（1024 长度不够而已）→ 在保守默认下
  这部分被**错误分派至 wrong 支**（1024 regime 下 truncated≈69% of rollouts → 粗估**约 1/3 训练数据
  被误判为 wrong**）。→ **动机记录**：把 **max_len（1024→2048）升为训练矩阵必跑消融轴**，2048 下截断率降、
  误分派减，truncated 可"直判"替代保守默认。
