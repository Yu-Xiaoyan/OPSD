...
## eval 自动挂载（授权 2026-07-15，ckpt 出即挂，无需逐次请示）
- 基线（立即）：base=33449 / 官方OPSD(B2)=33450。
- 训练臂 eval（`-W depend=afterok` 自动释放）：v2main=33451(←33443) / v2noB=33452(←33444) /
  v2noCp=33453(←33445) / v2w05=33454(←33446) / v2len2048=33455(←33447) / v2w0=33456(←33448)。
- 协议：AIME24/25 avg@12 + MATH500 avg@4 @ ckpt 50/100/150（平台期形状），锁定协议，`results/v2_eval/`。
- **全部分数落齐后一次性报表**：各臂终点分 + checkpoint 曲线 + 按锁死判读线的逐条判定草案
  （仅对照预注册，不超解读）。

## 判读表最终锁定（用户裁决 2026-07-15，唯一口径，取代此前"两种并列报"方案）

> 基线 = 官方 OPSD（B2 = 33450，纯 OPSD，复用 repro）。**报表按此表逐行出，不超预注册解读。**

| 行 | 判读 | 对比 | 判据 | 类型 |
|--:|---|---|---|---|
| 1 | **B 部件（主判读/主线句）** | **noCp（纯 B） vs 官方 OPSD(B2)** | AIME24 **≥+1.5** 且 MATH500 不降 **+ 平台期推后** | 判定 |
| 2 | **C′ 单独贡献** | noB（C′-only） vs 官方 OPSD(B2) | AIME24 **≥+1.5** 且 MATH500 不降（同构） | 判定（注：此对比为**配置差异的意外产物**，如实标注） |
| 3 | **C′ 边际（次判读）** | main vs noCp | **≥+1.0** → C′ 升格进方法；**<+1.0 或掉分** → 降级不写 | 判定（C′-1 强度轴嫌疑照记，有效须正面回应） |
| 4 | **机制判据** | noCp（纯 B） 相对 noB | **平台期推后** | 机制（不变） |
| 5 | **ω 轴** | w05 / w0 vs 官方 OPSD | 照原预注册（AIME24 ≥+1.5 且 MATH500 不降） | 判定 |
| 6 | **len2048 轴** | len2048 vs 官方 OPSD | 照原预注册 | 判定 |
| 7 | **协同检查** | main vs 官方 OPSD ↔ (B 增益 + C′ 增益) 加和 | **无过线判据** | exploratory（只描述叠加/冗余/拮抗形态） |
| — | 参考行（**不作部件判定**） | noCp vs noB | — | 参考 |

- **全臂不过线（全 AIME24 增益 < +1.5）→ A 路线预案，8/15 闸门不变。**
- 此前"noB(官方 OPSD)标注歧义"**已裁决解决**：B 主判读基线取 **官方 OPSD(B2)**，noCp-vs-noB 降为参考行。
