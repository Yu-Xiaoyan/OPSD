...
def main():
    ...
    llm = LLM(model=BASE, enable_lora=True, max_lora_rank=64, max_loras=1,
              ...
              enforce_eager=True)
    lora = LoRARequest("ckpt50", 1, CKPT)
    ...
    for max_tokens in args.max_tokens:
        ...
        out_path = os.path.join(OUT_DIR, f"rollouts_ckpt50_max{max_tokens}.jsonl")
        buckets = Counter()
        ...
        summ = {"max_tokens": max_tokens, "n": total, "buckets": dict(buckets),
                "gen_params": gen_params, "source_checkpoint": CKPT}
        with open(os.path.join(OUT_DIR, f"summary_max{max_tokens}.json"), "w") as f:
